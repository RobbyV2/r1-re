#!/usr/bin/env bun
// G2 EvenHub text synthesizer.
//
// Sends arbitrary text to a TextContainer ("cmux-text" by default) on the
// glasses. One Cmd=7 create/update message over sid=e0. Text is pre-formatted
// by the caller (50 cols × 10 rows at the fixed LVGL font is the usable area).
//
// Usage:
//   bun g2-text-send.ts --text "hello world"
//   bun g2-text-send.ts --file some.txt
//   bun g2-text-send.ts --text "..." --width 576 --height 288 --name cmux-text
...
// TextContainerConfig (inner-inner): fields observed in capture f7316+
//   f1 x, f2 y, f3 width, f4 height, f9 flag=1, f10 name, f11 flag2=0, f12 textBytes
function buildTextConfig(opts: {
  x: number; y: number; width: number; height: number;
  name: string; text: string;
  f9?: number; f11?: number;
}): Uint8Array {
  const out: number[] = [];
  ...
  out.push(...pbV(11, opts.f11 ?? 0));
  out.push(...pbS(12, opts.text));
  return new Uint8Array(out);
}

// UpdateContainerMsg wrapper: f1=1 (op), f3=TextContainerConfig (for text)
function buildUpdateContainerTextMsg(cfg: Uint8Array): Uint8Array {
  const out: number[] = [];
  out.push(...pbV(1, 1));
  out.push(...pbB(3, cfg));
  return new Uint8Array(out);
...
// Outer evenhub_main_msg_ctx: f1=7 (Cmd), f2=magic, f7=UpdateContainerMsg bytes
function buildOuterUpdateContainer(magic: number, msg: Uint8Array): Uint8Array {
  const out: number[] = [];
  out.push(...pbV(1, 7));
...
// Fragment a pb payload across BLE frames with EvenHub envelope.
...
// Prelude frame f5872 (sid=01 type=2 app-launch)
...
async function loadPreludeF5883(): Promise<Uint8Array> {
  // Path to your own captured btsnoop TSV. See guides/ble-protocol-playbook.md
  // for how to produce this with tshark. Set G2_CAPTURE_TSV to override.
  const tsv = process.env.G2_CAPTURE_TSV || "./captures/app_session.tsv";
  const t = await Bun.file(tsv).text();
  for (const line of t.split("\n")) {
    const c = line.split("\t");
    if (c[0] === "5883") {
      const hex = c[4]!.toLowerCase();
      const out = new Uint8Array(hex.length / 2);
      for (let i = 0; i < out.length; i++) out[i] = parseInt(hex.slice(i * 2, i * 2 + 2), 16);
      return out;
    }
  }
  throw new Error("f5883 not in capture");
}
...
async function main() {
  ...
  let text = argMap["text"] ?? "";
  if (argMap["file"]) text = await Bun.file(argMap["file"]!).text();
  if (!text) {
    text = [
      "  G2 TEXT TEST  ",
      "",
      "  50 cols x 10 rows",
      "  pushed via direct BLE",
      "  no SDK, no Even app",
      "",
  ...
  const width = parseInt(argMap["width"] || "576", 10);
  const height = parseInt(argMap["height"] || "288", 10);
  const name = argMap["name"] || "cmux-text";
  const skipPrelude = argMap["no-prelude"] === "true";

  console.log(`[${ts()}] g2-text-send: ${text.length}B text → ${name} (${width}×${height})`);
  ...
  if (!skipPrelude) {
    ...
    const f5883 = await loadPreludeF5883();
    console.log(`[${ts()}] prelude: f5883 (sid=e0 startup)`);
    const a2 = waitForAck(armA, 0xe0, 157, 3000);
    await armA.write.writeAsync(Buffer.from(f5883), true);
    await a2;
  }

  // Build text container update
  const cfg = buildTextConfig({ x: 0, y: 0, width, height, name, text });
  const updMsg = buildUpdateContainerTextMsg(cfg);
  const magic = 200;
  const outer = buildOuterUpdateContainer(magic, updMsg);
  console.log(`[${ts()}] outer pb=${outer.length}B`);
  const frames = buildBleFrames(outer, 0x30, 0xe0, 0x20);
  ...
  if (!ack) {
    ...
    // Cmd=8 (ack for cmd=7) response: f1=8, f2=magic, f8=bytes{f1 varint=code}
    // code 6 = SUCCESS
    let ec = -1;
    for (let k = 0; k < ack.length - 3; k++) {
      if (ack[k] === 0x42 && ack[k + 1] === 0x02 && ack[k + 2] === 0x08) {
        ec = ack[k + 3]!;
        break;
      }
    }
    console.log(`[${ts()}] ack ${ms.toFixed(0)}ms code=${ec} ${ec === 6 ? "SUCCESS" : "FAIL"}`);
  }

  console.log(`[${ts()}] holding 3s then disconnect`);
